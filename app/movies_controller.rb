...
def can_find_using_where_clause_and_be_sorted
  ...
  Movie.all.select{|m| m.release_date > 2002}.sort_by(&:release_date).reverse
end

def can_be_found_updated_and_saved
  # Updtate the title "Awesome Flick" to "Even Awesomer Flick", save it, then return it
  Movie.create(title: "Awesome Flick")
  updated_movie = Movie.all.find_by(title: "Awesome Flick")
...
def can_update_using_update_method
  ...
  updated_movie = Movie.all.find_by(title: "Wat?")
...
def can_destroy_a_single_item
  ...
  item = Movie.all.find_by(title: "That One Where the Guy Kicks Another Guy Once")
  item.destroy
end
